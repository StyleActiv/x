...
function updateCartDisplay() {
    ...
    cart.forEach(item => {
        ...
        if (item.isCustom && item.customization) {
            const { color, design, texto } = item.customization;
            detailsHTML = `<small style="color:#666;display:block;margin-top:2px;">Personalizado: color ${color}${design ? `, diseño ${design}` : ''}${texto ? `, texto "${texto}"` : ''}</small>`;
        }
        cartItem.innerHTML = `
...
function showCustomizationModal(productId) {
    ...
    const overlay = document.getElementById('modal-overlay');
    // Guardar metadata del modelo para filtrar colores disponibles
    const modelColorsMap = {
        'casco-1': ['negro', 'blanco', 'verde'],
        'casco-2': ['blanco', 'azul', 'rosado'],
        'casco-3': ['rojo', 'negro', 'rosado'],
        'casco-4': ['azul', 'negro', 'verde']
    };
    const availableColors = modelColorsMap[productId] || ['negro', 'blanco', 'rojo', 'azul', 'verde', 'rosado'];
    document.querySelectorAll('.color-btn').forEach(btn => {
        const allowed = availableColors.includes(btn.dataset.color);
    ...
    // Resetear opciones y estado de personalización
    const enableCheckbox = document.getElementById('enable-personalization');
    if (enableCheckbox) enableCheckbox.checked = false;
    document.getElementById('custom-text').value = '';
    const designSelect = document.getElementById('design-select');
    designSelect.value = 'sólido';
    document.getElementById('custom-text-group').style.display = 'none';
    setPersonalizationControlsEnabled(false);
    updateCustomPrice();
}
...
function addCustomizedToCart() {
    ...
    const enableCheckbox = document.getElementById('enable-personalization');
    const isPersonalized = enableCheckbox && enableCheckbox.checked;
    
    // Si no se selecciona personalización, agregar casco base
    if (!isPersonalized) {
        const baseNameEl = document.querySelector(`[data-product*="${currentCustomization}"] h3`);
        const baseName = baseNameEl ? baseNameEl.textContent : 'Casco';
        const basePrice = getProductPrice(currentCustomization);
        addToCart(currentCustomization, baseName, basePrice);
    ...
    const customText = (design === 'personalizado' && customTextInput) ? customTextInput.value.trim() : '';
    
    // Validaciones: color obligatorio, diseño obligatorio; texto visible solo si enable está activo
    if (!color) {
        showNotification('Selecciona un color para tu casco personalizado');
        return;
    }
    if (!design) {
        showNotification('Selecciona un diseño');
        return;
    }
    // Si el diseño es personalizado, exigir texto
    if (design === 'personalizado' && !customText) {
        showNotification('Ingresa el texto personalizado');
        return;
    }
    ...
    let productName = `${baseName} (Personalizado)`;
    const details = { color, design };
    if (customText) details.texto = customText;
    
    const basePrice = getProductPrice(currentCustomization);
    const customPrice = calculateCustomPrice(design, customText);
    const totalPrice = basePrice + customPrice;
    
    // Agregar al carrito incluyendo metadatos de personalización
    const itemId = `${currentCustomization}-custom-${color}-${design}${customText ? '-' + customText : ''}`;
    // Pasar imagen del producto base
    const baseImage = getProductImage(currentCustomization);
    addToCart(itemId, productName, totalPrice, baseImage);
    // Anexar metadata al último item agregado
    const added = cart.find(i => i.id === itemId);
    if (added) {
        added.isCustom = true;
        added.baseProductId = currentCustomization;
        added.customization = details;
    }
    closeCustomizationModal();
}
...
function updateCustomPrice() {
    ...
    const price = isPersonalized ? calculateCustomPrice(design, customText) : 0;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (enableCheckbox) {
        enableCheckbox.addEventListener('change', function() {
            ...
            setPersonalizationControlsEnabled(active);
            document.getElementById('custom-text-group').style.display = active ? 'block' : 'none';
            updateCustomPrice();
        });
    }
    ...
    if (designSelect) {
        designSelect.addEventListener('change', function() {
            // Habilitar texto solo cuando el diseño sea "personalizado"
            const isCustomDesign = this.value === 'personalizado';
            const textGroup = document.getElementById('custom-text-group');
            const input = document.getElementById('custom-text');
            if (textGroup && input) {
                textGroup.style.display = isCustomDesign ? 'block' : 'none';
                input.disabled = !isCustomDesign;
                if (!isCustomDesign) input.value = '';
            }
            updateCustomPrice();
        });
    }
...
// Habilitar/Deshabilitar controles de personalización
function setPersonalizationControlsEnabled(enabled) {
    const designSelect = document.getElementById('design-select');
    const customText = document.getElementById('custom-text');
    const colorButtons = document.querySelectorAll('.color-btn');
    if (designSelect) designSelect.disabled = !enabled;
    if (customText) {
        customText.disabled = !enabled;
        customText.value = enabled ? customText.value : '';
    }
    colorButtons.forEach(btn => { btn.disabled = !enabled || btn.style.display === 'none'; });
}
...
function buildCartLinesForMessage() {
    const lines = cart.map(item => {
        ...
        if (item.isCustom && item.customization) {
            ...
            if (design) parts.push(`diseño ${design}`);
            if (texto) parts.push(`texto "${texto}"`);
            if (parts.length) details = ` [${parts.join(', ')}]`;
        }
        return `- ${item.name}${details} x${item.quantity} - $${lineTotal.toLocaleString()} COP`;
    });
...
function submitReservation(event) {
    ...
    const message = `¡Hola!  🛍️ Quiero Reservar Mi Compra 🛍️.\n\n${customerBlock}\nMétodo de pago 💳 :  ${paymentMethod}\n\nProductos seleccionados 🛒 :   \n${productsBlock}\n\nTotal 💰 :   $${cartTotal.toLocaleString()} COP`;
...
function updateWhatsAppCompatibilityInfo() {
    ...
    if (isMobile) {
        infoText.innerHTML = `
            • <strong>Con app:</strong>Para seguir la compra, Deberas darle permisos para abrir whatsapp.<br>
        
        `;
    } else {
        infoText.innerHTML = `
            • <strong>Portatil :</strong> Para seguir la compra, En la siguiente ventana deberas hacer click en [ Permitir o  Abrir ] en whatsapp<br>
        `;
    }
}
